...
We tune numerous settings with puppet code that ships with Puppet Enterprise.  So, the settings are in a good position and likely only need to be changed during the course of a specific performance issue.  

That leaves us with items that are not currently handled by Puppet Enterprise.  
...
You can improve the performance ( reduce time to completion ) of maintenance tasks by increasing the maintenance_work_mem setting.  
